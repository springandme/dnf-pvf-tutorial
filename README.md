...
DAF 源站屏蔽境外 IP，海外服务器直连回源会失败，但**不影响已缓存的图片**：
`/media/` 请求优先读本地 `media-cache/`，未命中才尝试回源，失败由前端
显示占位符。因此海外部署的推荐流程是在国内网络的机器上完成预热，再
把缓存目录同步过去：

```bash
# 国内机器：预热全部图片（断点续传，可反复跑）
node scripts/mirror-daf-media.js

# 同步到部署服务器（约 243MB）
tar -C . -cf - media-cache | ssh <server> 'tar -C /path/to/deploy -xf -'
```
